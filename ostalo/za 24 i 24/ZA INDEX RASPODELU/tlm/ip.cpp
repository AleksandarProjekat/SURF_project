...
void Ip::proc() {
    ...
    struct stat info;
    if (stat("index_upper", &info) != 0) {
        mkdir("index_upper", 0777);  // Create the directory
    }
    if (stat("index_lower", &info) != 0) {
        mkdir("index_lower", 0777);  // Create the directory
    }
    ...
    else if (start == 0 && ready == 0)
    {
        ...
        for (int i = 0; i < _IndexSize * _IndexSize * 4; i++)
        {
         ...
         delete[] _Pixels; 
        std::ostringstream file_upper, file_lower;
    file_upper << "index_upper/index_upper_" << iteration << ".txt";
    file_lower << "index_lower/index_lower_" << iteration << ".txt";

    std::ofstream file_up(file_upper.str());
    std::ofstream file_low(file_lower.str());

    if (!file_up.is_open() || !file_low.is_open()) {
        SC_REPORT_ERROR("Ip", "Could not open file for writing.");
        return;
    }

    // Function to split num_f into upper and lower 24 bits
    auto split_num_f = [](num_f value, uint32_t& upper, uint32_t& lower) {
        uint64_t temp = static_cast<uint64_t>(value * (1 << 18)); // Convert to integer with decimals
        upper = static_cast<uint32_t>((temp >> 24) & 0xFFFFFF);  // Extract upper 24 bits
        lower = static_cast<uint32_t>(temp & 0xFFFFFF); // Extract lower 24 bits
    };

    // Loop over the _index array and write upper and lower 24 bits to respective files
    for (int i = 0; i < _IndexSize; i++) {
        for (int j = 0; j < _IndexSize; j++) {
            for (int k = 0; k < 4; k++) {
                uint32_t upper, lower;
                split_num_f(_index[i][j][k], upper, lower);
                
                // Write to the upper and lower files
                file_up << upper << "\n";
                file_low << lower << "\n";
            }
        }
