...
void Ip::proc() {
    static int iteration = 0; // To track the iteration
    std::ostringstream file_name;
    file_name << "output_folder/parameters_input" << iteration << ".txt";

    std::ofstream file(file_name.str());
    if (!file.is_open()) {
        SC_REPORT_ERROR("Ip", "Could not open file for writing.");
        return;
    }

// Funkcija koja deli 48-bitni num_f na gornjih 24 bita i donjih 24 bita
auto split_num_f = [](num_f value, uint32_t& upper, uint32_t& lower) {
    // Skaliraj num_f (koji je 48-bitni fiksni broj) tako da ima celobrojni i decimalni deo
    uint64_t temp = static_cast<uint64_t>(value * (1 << 18)); // Pretvori u celobrojni sa decimalima

    // Gornjih 24 bita
    upper = static_cast<uint32_t>((temp >> 24) & 0xFFFFFF);  // Izvuci gornjih 24 bita
    
    // Donjih 24 bita
    lower = static_cast<uint32_t>(temp & 0xFFFFFF); // Izvuci donjih 24 bita
};

uint32_t fracr_upper, fracr_lower, fracc_upper, fracc_lower;
uint32_t spacing_upper, spacing_lower, i_cose_upper, i_cose_lower;
uint32_t i_sine_upper, i_sine_lower;

split_num_f(fracr, fracr_upper, fracr_lower);
split_num_f(fracc, fracc_upper, fracc_lower);
split_num_f(inv_spacing, spacing_upper, spacing_lower);
split_num_f(_cose, i_cose_upper, i_cose_lower);
split_num_f(_sine, i_sine_upper, i_sine_lower);

    // Output to file in the required format
    file <<  fracr_upper << "\n";
    file << fracr_lower << "\n";
    file <<  fracc_upper << "\n";
    file <<  fracc_lower << "\n";
    ...
    // For num_i variables (iradius, iy, ix, step, scale)
    file << iradius << "\n";
    file <<iy << "\n";
    file << ix << "\n";
    file <<step << "\n";
